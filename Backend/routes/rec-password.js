...
router.post('/api/rec-password', async (req, res)=>{
    ...
    if(UserR = await user.findOne({email:email_l}) || (UserDev = await userdev.findOne({email:email_l}))) {
        /*if(UserDev.estado=='inactivo'){
        ...
        await userdev.updateOne({email:email_l},{$set:{temporal_pass:hash}});
