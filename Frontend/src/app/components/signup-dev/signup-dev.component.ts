...
@Component({
  selector: 'abe-signup-dev',
  templateUrl: './signup-dev.component.html',
  styleUrls: ['./signup-dev.component.scss']
})
export class SignupDevComponent implements OnInit {

  signupd: FormGroup = new FormGroup({
  ...
  hide = true;
  hide2= true;
  get emailInput() { return this.signupd.get('email'); }
  get passwordInput() { return this.signupd.get('password'); }
  get vpasswordInput() { return this.signupd.get('vpassword'); }
  ...
  verify(){
    var pass= this.signupd.value.password;
    if(pass.length<8){
  ...
  /*}else if(!this.user.email.match(/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3,4})+$/)){
    ...
    this.authService.signUpDev(this.user)
    .subscribe(
      res =>{
        if(res.estado=='usuario_repetido'){
          ...
          Swal.fire("Registro Exitoso", "Bienvenido a Paratus, revisa tu bandeja de entrada en el correo institucional y sigue los pasos para activar tu cuenta", "success");
      ...
      err =>{
        ...
        this.router.navigate(['/signup-dev']);
      }
    )
  }
