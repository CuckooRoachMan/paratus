...
@Component({
  selector: 'abe-edit-profile-dev',
  templateUrl: './edit-profile-dev.component.html',
  styleUrls: ['./edit-profile-dev.component.scss'],
  providers: [DatePipe]
})
export class EditProfileDevComponent implements OnInit {
 ...
 showPreview(event) {
  ...
  actualizarPerfil(){
    ...
    this.authService.editProfileDev(this.profile)
    .subscribe(
      res =>{
        if(res.estado=='Hecho'){
  ...
  goSubirImagen(){
      ...
      this.authService.uploadProfileDev(this.fd,this.profile.email)
      .subscribe( res => {
        if(res.estado=='Hecho'){
